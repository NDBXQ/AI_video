...
const inputSchema = z.object({
  user_requirement: z.string().trim().min(1).max(50_000)
})
...
export async function POST(req: Request): Promise<Response> {
  ...
  logger.info({
    event: "coze_short_drama_planning_start",
    module: "coze",
    traceId,
    message: "开始短剧剧本策划"
  })
  ...
  if (!parsed.success) {
    logger.warn({
      event: "coze_short_drama_planning_validation_failed",
      module: "coze",
      traceId,
      message: "短剧剧本策划入参校验失败"
    })
  ...
  const url = readEnv("SHORT_DRAMA_PLANNING_API_URL")
  const token = readEnv("SHORT_DRAMA_PLANNING_API_TOKEN")
  if (!url || !token) {
    return NextResponse.json(
      makeApiErr(
        traceId,
        "COZE_NOT_CONFIGURED",
        "Coze 未配置，请设置 SHORT_DRAMA_PLANNING_API_URL 与 SHORT_DRAMA_PLANNING_API_TOKEN"
      ),
      { status: 500 }
    )
  }
  ...
  try {
    const coze = await callCozeRunEndpoint({
      ...
      token,
      body: parsed.data,
      module: "coze_short_drama_planning"
    })

    const durationMs = Date.now() - start
    logger.info({
      event: "coze_short_drama_planning_success",
      module: "coze",
      traceId,
      message: "短剧剧本策划成功",
      durationMs,
    ...
    if (err instanceof CozeRunEndpointError) {
      logger.error({
        event: "coze_short_drama_planning_failed",
        module: "coze",
        traceId,
        message: "短剧剧本策划失败（Coze 调用失败）",
        durationMs,
        status: err.status
      })
      return NextResponse.json(
    ...
    logger.error({
      event: "coze_short_drama_planning_error",
      module: "coze",
      traceId,
      message: "短剧剧本策划异常",
      durationMs,
