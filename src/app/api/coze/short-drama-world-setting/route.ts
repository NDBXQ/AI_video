...
export async function POST(req: Request): Promise<Response> {
  ...
  logger.info({
    event: "coze_short_drama_world_setting_start",
    module: "coze",
    traceId,
    message: "开始生成短剧世界观设定"
  })
  ...
  if (!parsed.success) {
    logger.warn({
      event: "coze_short_drama_world_setting_validation_failed",
      module: "coze",
      traceId,
      message: "短剧世界观设定入参校验失败"
    })
  ...
  const url = readEnv("SHORT_DRAMA_WORLD_SETTING_API_URL")
  const token = readEnv("SHORT_DRAMA_WORLD_SETTING_API_TOKEN")
  if (!url || !token) {
    return NextResponse.json(
      makeApiErr(
        traceId,
        "COZE_NOT_CONFIGURED",
        "Coze 未配置，请设置 SHORT_DRAMA_WORLD_SETTING_API_URL 与 SHORT_DRAMA_WORLD_SETTING_API_TOKEN"
      ),
      { status: 500 }
    )
  }
  ...
  try {
    const coze = await callCozeRunEndpoint({
      ...
      token,
      body: parsed.data,
      module: "coze_short_drama_world_setting"
    })

    const durationMs = Date.now() - start
    logger.info({
      event: "coze_short_drama_world_setting_success",
      module: "coze",
      traceId,
      message: "短剧世界观设定生成成功",
      durationMs,
    ...
    if (err instanceof CozeRunEndpointError) {
      logger.error({
        event: "coze_short_drama_world_setting_failed",
        module: "coze",
        traceId,
        message: "短剧世界观设定生成失败（Coze 调用失败）",
        durationMs,
        status: err.status
      })
      return NextResponse.json(
    ...
    logger.error({
      event: "coze_short_drama_world_setting_error",
      module: "coze",
      traceId,
      message: "短剧世界观设定生成异常",
      durationMs,
