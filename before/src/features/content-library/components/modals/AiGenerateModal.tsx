...
export function AiGenerateModal({
  ...
  const submit = async () => {
    try {
      setSubmitting(true);
      setError(null);
      const res = await fetch('/api/content-library/assets/ai-generate', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ prompt: prompt.trim(), quantity }),
      });
    ...
    <div className="fixed inset-0 z-50">
      ...
      <div className="absolute left-1/2 top-1/2 w-[calc(100%-2rem)] max-w-xl -translate-x-1/2 -translate-y-1/2 overflow-hidden rounded-2xl bg-white shadow-2xl">
        <div className="flex items-center justify-between border-b border-gray-100 px-5 py-4">
          <div className="text-sm font-semibold text-gray-900">AI 生成图片</div>
        ...
        <div className="space-y-4 p-5">
          <div className="space-y-2">
            ...
            <textarea
              ...
              placeholder="例如：现代城市夜景，霓虹灯，赛博朋克风格，高清"
              rows={4}
